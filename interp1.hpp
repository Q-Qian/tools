...
//  Test
//
//  Created by 乔迁 on 2020/8/7.
//  Copyright © 2020 乔迁. All rights reserved.
//
...
int getPchipIterp(double *x, double *y, int len, double *outX, double *outY, int outlen)
{
    double S1 = -1;
    double S2 = 1;
    ...
    int j=0;
    for (int k=0;k<outlen;k+=1)   //求拟合值，步进为0.1
    {
        double tmpX = outX[k];
        for (int i=0;i<n;i++)
        {
            if (tmpX>=x[i]&&tmpX<=x[i+1])
            {
                j=i;
                break;
            }
        }
        double r1=x[j+1]-tmpX;
        double r2=tmpX-x[j];
        double tmpY=(pow(r1,3.0)/(6.0*h[j]))*m[j]+
